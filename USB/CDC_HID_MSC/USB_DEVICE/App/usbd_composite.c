...
USBD_CUSTOM_HID_HandleTypeDef pClassDataHID;
...
static uint8_t USBD_COMPOSITE_CfgDesc[USB_COMPOSITE_CONFIG_DESC_SIZ] =
{
    ...
    0x02,   // bFunctionSubClass：子类型
    0x01,   // bFunctionProtocol：控制协议
    0x00,   // iFunction
...
//    pClassDataHID = pdev->pClassData;
    /* MSC */
    pdev->pClassData = &pClassDataMSC;
    pdev->pUserData = &USBD_Storage_Interface_fops_FS;
    ret += USBD_MSC.Init(pdev, cfgidx);
    return ret;
}
                                      ...
                                      uint8_t cfgidx)
{
    ...
    pdev->pClassData = &pClassDataMSC;
    pdev->pUserData = &USBD_Storage_Interface_fops_FS;
    ret += USBD_MSC.DeInit(pdev, cfgidx);
    return ret;
}
