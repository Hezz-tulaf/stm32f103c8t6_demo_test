...
__ALIGN_BEGIN static uint8_t USBD_HID_CfgFSDesc[USB_HID_CONFIG_DESC_SIZ] __ALIGN_END =
{
  ...
  0x65, 0x00,                     // wTotalLength: 101字节（0x65）
  0x03,                           // bNumInterfaces: 3个接口
  0x01,                           // bConfigurationValue: 配置1
  0x00,                           // iConfiguration: 无字符串描述符
  ...
  0x00,                           // bCountryCode: 无
  0x01,                           // bNumDescriptors: 1个报告描述符
  0x22,                           // bDescriptorType: 报告描述符
  HID_MOUSE_REPORT_DESC_SIZE, 0x00, // wDescriptorLength: 报告描述符长度

  // 端点描述符
  0x07,                           // bLength: 7字节
  USB_DESC_TYPE_ENDPOINT,         // bDescriptorType: 端点
  HID_EPIN_ADDR_MOUSE1,           // bEndpointAddress: IN端点1 (0x81)
  0x03,                           // bmAttributes: 中断传输
  HID_EPIN_SIZE, 0x00,            // wMaxPacketSize: 包大小
  ...
  0x07,                           // bLength: 7字节
  USB_DESC_TYPE_ENDPOINT,         // bDescriptorType: 端点
  HID_EPIN_ADDR_MOUSE2,           // bEndpointAddress: IN端点2 (0x82)
  0x03,                           // bmAttributes: 中断传输
  HID_EPIN_SIZE, 0x00,            // wMaxPacketSize: 包大小
  HID_FS_BINTERVAL,               // bInterval: 轮询间隔
  ...
  0x01,                           // bNumDescriptors: 1个报告描述符
  0x22,                           // bDescriptorType: 报告描述符
  HID_MOUSE_REPORT_DESC_SIZE, 0x00, // wDescriptorLength: 键盘报告描述符长度

  // 端点描述符
  0x07,                           // bLength: 7字节
  USB_DESC_TYPE_ENDPOINT,         // bDescriptorType: 端点
  HID_EPIN_ADDR_KEYBOARD,         // bEndpointAddress: IN端点3 (0x83)
  0x03,                           // bmAttributes: 中断传输
  HID_EPIN_SIZE, 0x00,            // wMaxPacketSize: 包大小
  HID_FS_BINTERVAL                // bInterval: 轮询间隔
};
...
////  0x01,   0xc0
//};
__ALIGN_BEGIN static uint8_t HID_KEYBOARD_ReportDesc[HID_MOUSE_REPORT_DESC_SIZE]  __ALIGN_END =
{
	0x05, 0x01,// USAGE_PAGE (Generic Desktop)
		0x09, 0x06,// USAGE (Keyboard)
			...
			//65
	0x05, 0x01, // USAGE_PAGE (Generic Desktop)
		0x09, 0x02, // USAGE (Mouse) //这是一个局部（bType为2）条目，说明接下来的应用集合用途用于鼠标
			...
			0x09, 0x01, //   USAGE (Pointer)//这是一个局部条目。说明用途为指针集合
	...
	0x05, 0x01, // USAGE_PAGE (Generic Desktop)
		...
		//普通桌面用的鼠标。
			0xa1, 0x01, // COLLECTION (Application)    //1byte报告ID +按键(3bit)+填充行(5bit)=1byte ; 坐标（16bits*2个）=4bytes；所以上报数据就是6bytes；  
			0x85,0x03,   // 报告ID（报告ID 0是保留的），多个设备需要添加。
			//这是一个局部条目。说明用途为指针集合
